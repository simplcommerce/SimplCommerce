...
namespace SimplCommerce.Module.SampleData.Services
{
    public class SampleDataService : ISampleDataService
    {
        ...
        public async Task ResetToSampleData(SampleDataOption model)
        {
            ...
            var useMySql = _sqlRepository.GetDbConnectionType()
                .Contains("MySql", StringComparison.InvariantCultureIgnoreCase);
            var sampleContentFolder = Path.Combine(GlobalConfiguration.ContentRootPath, "Modules",
                "SimplCommerce.Module.SampleData", "SampleContent", model.Industry);
            ...
            var commands = usePostgres || useSQLite || useMySql
                ? _sqlRepository.PostgresCommands(lines)
