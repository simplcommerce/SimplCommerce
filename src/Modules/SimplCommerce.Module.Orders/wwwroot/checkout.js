$(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.shippingPrices.length > 0) {
                    $.each(data.shippingPrices,
                        function(index, value) {
                            $shippingMethods.append('<div class="radio"> \
                        <label> \
                        <input type="radio" name="ShippingMethod" data-price ="' +
                                value.priceText +
                                '" value="' +
                                value.name +
                                '"> \
    ...
    $("input[name=ShippingAddressId]").on("change",
        function() {
            toggleCreateShippingAddress();
        });

    $("input[name=UseShippingAddressAsBillingAddress]").on("change",
        function() {
            toggleChangeBillingAddress();
            toggleCreateBillingAddress();
        });

    $("input[name=BillingAddressId]").on("change",
        function() {
            toggleCreateBillingAddress();
        });

    $(document).on("change",
        "input[name=ShippingAddressId], #NewAddressForm_StateOrProvinceId, #NewAddressForm_DistrictId, #NewAddressForm_ZipCode, #shippingMethods input:radio",
        function() {
            updateShippingInfo();
        });
    ...
    $("#NewBillingAddressForm_CountryId").on("change",
        function() {
            var countryId = this.value;
            $("#NewBillingAddressForm_ZipCode").val("");

            $.getJSON("/api/country-states-provinces/" + countryId,
                function(data) {
                    ...
                    $.each(data.statesOrProvinces,
                        function(index, option) {
                            $stateOrProvinceSelect.append($("<option></option>").attr("value", option.id)
                                .text(option.name));
                        });
    ...
    $("#NewAddressForm_CountryId").on("change",
        function() {
            var countryId = this.value;
    ...
    $("#NewBillingAddressForm_StateOrProvinceId").on("change",
        function() {
            var selectedStateOrProvinceId = this.value;

            $.getJSON("/api/states-provinces/" + selectedStateOrProvinceId + "/districts",
                function(data) {
                    var $districtSelect = $("#NewBillingAddressForm_DistrictId");
                    resetSelect($districtSelect);
    ...
    $("#NewAddressForm_StateOrProvinceId").on("change",
        function() {
            var selectedStateOrProvinceId = this.value;
