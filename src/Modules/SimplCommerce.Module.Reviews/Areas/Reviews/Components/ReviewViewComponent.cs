...
namespace SimplCommerce.Module.Reviews.Areas.Reviews.Components
{
    public class ReviewViewComponent : ViewComponent
    {
        ...
        public ReviewViewComponent(IRepository<Review> reviewRepository, IWorkContext workContext,
        ...
        {
            var model = new ReviewVm {EntityId = entityId, EntityTypeId = entityTypeId};
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                var query = _orderItemRepository.Query().Where(x =>
                    x.Order.CustomerId == user.Id && (x.Product.Id == entityId ||
                                                      x.Product.LinkedProductLinks.Any(i => i.Product.Id == entityId)));
                if (query.Count() > 0)
                {
