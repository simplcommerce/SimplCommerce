...
namespace SimplCommerce.Module.Core.Services
{
    public class ThemeService : IThemeService
    {
        ...
        private readonly string _currentThemeName;
        ...
        {
            var themeSetting = await _appSettingRepository.Query().Where(x => x.Id == SimplConstants.ThemeConfigKey)
                .FirstAsync();
        ...
        public string PackTheme(string themeName)
        {
            ...
            var themeFolderWWWroot =
                new DirectoryInfo(Path.Combine(GlobalConfiguration.WebRootPath, "themes", themeName));

            var tempFolderName = Guid.NewGuid().ToString();
            var tempDir =
                Directory.CreateDirectory(Path.Combine(GlobalConfiguration.ContentRootPath, "Temps", tempFolderName));
            DirectoryCopy(themeFolder.FullName, Path.Combine(tempDir.FullName, "Themes", themeName), true);
            DirectoryCopy(themeFolderWWWroot.FullName, Path.Combine(tempDir.FullName, "wwwroot", "themes", themeName),
                true);

            var destinationArchiveFileName =
                Path.Combine(GlobalConfiguration.ContentRootPath, "Temps", $"{tempFolderName}.zip");
        ...
        private static void DirectoryCopy(string sourceDirName, string destDirName, bool copySubDirs)
        {
            ...
            if (copySubDirs)
            {
                ...
                foreach (var subdir in sourceSubDirectories)
                {
