...
namespace SimplCommerce.Module.PaymentMomo.Areas.PaymentMomo.Controllers
{
    ...
    public class MomoPaymentController : Controller
    {
        ...
        private readonly Lazy<MomoPaymentConfigForm> _setting;
        ...
        public async Task<IActionResult> MomoCheckout()
        {
            ...
            var orderCreateResult =
                await _orderService.CreateOrder(cart.Id, "MomoPayment", 0, OrderStatus.PendingPayment);
            ...
            var paymentRequest = new PaymentSubmitRequest(
                _setting.Value.SecretKey,
                _setting.Value.PartnerCode,
                _setting.Value.AccessKey,
                orderCreateResult.Value.OrderTotal,
                orderCreateResult.Value.Id,
                "",
                $"{rootUrl}/momo/result",
                $"{rootUrl}/momo/notify",
                ""
            ...
            var momoUrl = _setting.Value.IsSandbox
        ...
        {
            var momoPaymentProvider = _paymentProviderRepository.Query()
                .FirstOrDefault(x => x.Id == PaymentProviderHelper.MomoPaymentProviderId);
            var momoSetting =
                JsonConvert.DeserializeObject<MomoPaymentConfigForm>(momoPaymentProvider.AdditionalSettings);
            return momoSetting;
        }
